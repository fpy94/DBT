...
def train(lr,bs,eps,cv,modeldir,savedir,FOREST_NUM,MAX_EPOCHS,savecheck=25):
    ...
    if modeldir:
        ...
        with open('./pretrain_model/pretrain.pkl','rb') as f:
            models=pickle.load(f)
        net.load_state_dict(models[cv])
        nowep=0
    #now train the tree 
    ...
    #         print('building tree')
        ...
        print('Epoch {}, loss {}, pr {}, using time {}'.format((epochs+1),Loss,pr,etime-stime))

        if (epochs+1)%savecheck==0 or (epochs+1)==MAX_EPOCHS:
            torch.save({'epochs':epochs+1,
                        'model_state_dict':net.state_dict(),
                        'optimizer_state_dict':optimizer.state_dict()},
                        os.path.join(savedir,'checkpoint_cv_'+str(cv)+'_epoch_'+str(epochs+1)+'_bs_'+str(bs)+'_lr_'+str(lr)+'_rep_'+str(rep)+'.pkl')
                      )
...
if __name__=='__main__':
    ...
    parser.add_argument('-savedir','--save_dir',default='model_output',type=str,help='saving dir')
    ...
    parser.add_argument('-maxepoch','--max_epochs',default=25,type=int,help='max_epochs')
